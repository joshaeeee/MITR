...
static size_t ring_write_locked(mitr_preconnect_audio_src_t *src, const uint8_t *data, size_t bytes)
{
    ...
    if (src->dropped_bytes > 0 && !src->overflow_logged) {
        ...
        ESP_LOGW(TAG, "[PREROLL] preconnect_ring_overflow_dropped=%u", (unsigned)(src->dropped_bytes / sizeof(int16_t)));
    }
    return bytes;
}
...
static void preconnect_capture_task(void *arg)
{
    ...
    while (true) {
        ...
        /* Fan out the freshly captured frame to registered taps (e.g. the
         * wake-word detector). Snapshot the tap list under a separate short-
         * lived lock so tap callbacks themselves run without holding any
         * lock — they must not call back into this source. */
...
static esp_capture_err_t src_get_support_codecs(esp_capture_audio_src_if_t *h, const esp_capture_format_id_t **codecs, uint8_t *num)
{
...
static esp_capture_err_t src_negotiate_caps(esp_capture_audio_src_if_t *h, esp_capture_audio_info_t *in_caps, esp_capture_audio_info_t *out_caps)
{
    mitr_preconnect_audio_src_t *src = (mitr_preconnect_audio_src_t *)h;
