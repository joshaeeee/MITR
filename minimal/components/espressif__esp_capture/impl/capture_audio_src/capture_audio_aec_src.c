...
// Reference signal provided by main/media.c. Called to fill the R channel of
// the interleaved "MR" frame fed to the AFE so true AEC can operate.
extern void media_read_reference_pcm(int16_t *buf, int n_samples, int delay_samples);

// Notifies the session_timeout module that mic voice activity was detected.
// Implemented in media.c; called here to avoid a circular dependency.
extern void media_notify_mic_activity(void);
#define MIC_ACTIVITY_RMS_THRESHOLD 500  // ~0.015 of full scale; tune if needed

// Upper bound on AFE chunk size (samples). At 16 kHz the AFE uses 32 samples
// per frame; 128 gives ample headroom if that ever changes.
#define AEC_REF_CHUNK_MAX 128
...
static int open_afe_in_ram(void *arg)
{
    ...
    do {
        ...
        // AFE_TYPE_VC = Voice Communication: AEC + NS, no wake-word gating.
        // AFE_TYPE_SR gates fetch() on wake-word detection — wrong for a call pipeline.
        afe_config_t *afe_config = afe_config_init(src->mic_layout, src->models, AFE_TYPE_VC, AFE_MODE_LOW_COST);
...
{
    uint8_t src_ch = get_src_channel(src);
    static uint32_t s_feed_count = 0;
    s_feed_count++;

    if (src_ch == 2) {
        // mic_layout is "MR": ch0 = Mic, ch1 = Reference.
        // The hardware only has one mic so ch1 is unused/garbage from I2S RX.
        // Overwrite ch1 with the delayed playback reference so the AFE can
        // perform true acoustic echo cancellation.
        int n_frames = feed_size / (src_ch * (int)sizeof(int16_t));
        if (n_frames > AEC_REF_CHUNK_MAX) {
            n_frames = AEC_REF_CHUNK_MAX;
        }
        int16_t ref[AEC_REF_CHUNK_MAX];
        int sample_rate = src->info.sample_rate > 0 ? src->info.sample_rate : 16000;
        int delay_samples = (CONFIG_MITR_AEC_REFERENCE_DELAY_MS * sample_rate) / 1000;
        media_read_reference_pcm(ref, n_frames, delay_samples);
        int16_t *pcm = (int16_t *)feed_data;

        // Log every ~1 second and notify mic activity for session timeout.
        {
            int32_t mic_rms = 0;
            for (int i = 0; i < n_frames; i++) {
                mic_rms += (int32_t)pcm[i * 2] * pcm[i * 2];
            }
            mic_rms = n_frames > 0 ? mic_rms / n_frames : 0;
            // Notify session_timeout on voice activity
            if (mic_rms > (int32_t)MIC_ACTIVITY_RMS_THRESHOLD * MIC_ACTIVITY_RMS_THRESHOLD) {
                media_notify_mic_activity();
            }
            if (s_feed_count % 500 == 0) {
                int32_t ref_rms = 0;
                for (int i = 0; i < n_frames; i++) {
                    ref_rms += (int32_t)ref[i] * ref[i];
                }
                ref_rms = n_frames > 0 ? ref_rms / n_frames : 0;
                ESP_LOGI(TAG, "[AEC-FEED] #%lu ch=%d n_frames=%d mic_rms=%ld ref_rms=%ld delay_samp=%d",
                         (unsigned long)s_feed_count, src_ch, n_frames,
                         (long)mic_rms, (long)ref_rms, delay_samples);
                if (ref_rms == 0) {
                    ESP_LOGW(TAG, "[AEC-FEED] ref is SILENT — renderer callback not firing or buffer empty");
                }
            }
        }
        ...
        // Unexpected channel count — log once so we catch mis-configuration
        static bool s_ch_warned = false;
        if (!s_ch_warned) {
            ESP_LOGW(TAG, "[AEC-FEED] src_ch=%d — expected 2 (MR). Reference injection skipped!", src_ch);
            s_ch_warned = true;
        }
    }
...
static esp_capture_err_t audio_aec_src_start(esp_capture_audio_src_if_t *h)
{
    ...
    /* AFE fetch() output can be larger than the feed chunksize.
     * On ESP32-S3 the fetch returns 1024 bytes regardless of feed size.
     * Allocate at least 2048 bytes to avoid silent audio drops. */
    int min_cache = audio_chunksize * (16 / 8);
    src->cache_size = (min_cache < 2048) ? 2048 : min_cache;
    if (src->data_on_vad) {
...
static esp_capture_err_t audio_aec_src_read_frame(esp_capture_audio_src_if_t *h, esp_capture_stream_frame_t *frame)
{
    ...
    while (need_size > 0) {
        ...
        if (use_silent == false) {
            ...
            int copy_size = res->data_size;
            if (copy_size > src->cache_size) {
                ESP_LOGW(TAG, "AFE fetch %d > cache %d, truncating", res->data_size, src->cache_size);
                copy_size = src->cache_size;
            }
            memcpy(src->cached_frame, res->data, copy_size);
            src->cache_fill = copy_size;
        }
    }
