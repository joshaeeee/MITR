...
function SettingsItem({ icon, label, subtitle, onPress, danger, rightElement }: SettingsItemProps) {
  ...
  return (
    <TouchableOpacity style={styles.settingsItem} onPress={onPress} activeOpacity={0.6}>
      <View style={styles.settingsItemLeft}>
        {icon}
        <View>
          <Text style={[styles.settingsItemLabel, { color: colors.text }, danger && { color: colors.danger }]}>
            {label}
          </Text>
          {subtitle && <Text style={[styles.settingsItemSubtitle, { color: colors.textTertiary }]}>{subtitle}</Text>}
        </View>
      </View>
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <PastelCard color="mint" style={styles.profileCard}>
              <View style={styles.profileRow}>
                <View style={[styles.profileAvatar, { backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.6)' }]}>
                  <Text style={[styles.profileInitial, { color: colors.mintDark }]}>
                    {currentUser?.name?.charAt(0) ?? 'F'}
                  </Text>
                </View>
                <View style={styles.profileInfo}>
                  <Text style={[styles.profileName, { color: colors.text }]}>{currentUser?.name ?? 'Family user'}</Text>
                  <Text style={[styles.profileRole, { color: colors.textSecondary }]}>Owner · {currentUser?.email ?? '-'}</Text>
                </View>
              </View>
            </PastelCard>
            ...
            <PastelCard color="lavender" style={styles.elderCard}>
              <View style={styles.elderRow}>
                <View style={[styles.elderAvatar, { backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.6)' }]}>
                  <Text style={[styles.elderInitial, { color: colors.lavenderDark }]}>
                    {elderProfile?.name?.charAt(0) ?? 'E'}
                  </Text>
                </View>
                <View style={styles.elderInfo}>
                  <Text style={[styles.elderName, { color: colors.text }]}>{elderProfile?.name ?? 'Elder profile'}</Text>
                  <Text style={[styles.elderMeta, { color: colors.textSecondary }]}>
            ...
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: colors.textTertiary }]}>Care settings</Text>
              <PastelCard color="white" style={styles.sectionCard} padded={false}>
                <SettingsItem
            ...
            <View style={styles.section}>
              ...
              <PastelCard color="white" style={styles.sectionCard} padded={false}>
                <SettingsItem
                  icon={<Moon size={20} color={colors.lavenderDark} />}
                  label="Dark mode"
            ...
            <Text style={[styles.version, { color: colors.textTertiary }]}>MITR Family v1.0.0</Text>
      ...
      <Modal visible={showSignOut} transparent animationType="none">
        <Animated.View style={[styles.modalOverlay, { opacity: modalFade, backgroundColor: colors.overlay }]}>
          <Pressable style={styles.modalBackdrop} onPress={closeSignOutModal} />
          <Animated.View style={[styles.modalContent, { backgroundColor: colors.surface, transform: [{ translateY: modalSlide }] }]}>
            <View style={styles.modalIllustration}>
              <View style={[styles.modalIlluBlob1, { backgroundColor: colors.lavenderLight }]} />
              <View style={[styles.modalIlluBlob2, { backgroundColor: colors.peachLight }]} />
              <View style={[styles.modalIconWrap, { backgroundColor: isDark ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.85)' }]}>
            ...
            <Text style={[styles.modalTitle, { color: colors.text }]}>Leaving so soon?</Text>
            <Text style={[styles.modalSubtitle, { color: colors.textSecondary }]}>
              You will be signed out. Your family care data stays safe and you can sign back in anytime.
            </Text>
            <TouchableOpacity style={[styles.modalSignOutBtn, { backgroundColor: colors.danger }]} onPress={handleSignOut} activeOpacity={0.8}>
            ...
            <TouchableOpacity style={styles.modalCancelBtn} onPress={closeSignOutModal} activeOpacity={0.7}>
              <Text style={[styles.modalCancelText, { color: colors.textSecondary }]}>Stay signed in</Text>
            </TouchableOpacity>
          </Animated.View>
        </Animated.View>
      </Modal>
