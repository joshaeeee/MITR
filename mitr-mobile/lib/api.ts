...
export const bootstrapApiSession = async (): Promise<void> => {
  await loadSessionFromStorage();
  await ensureAuthenticated();
};

export const prefetchBootstrapData = async (): Promise<void> => {
...
export type HomeSummaryPayload = {
  ...
  deviceData: { status: ElderStatusSnapshot; device: DeviceInfo };
  alerts: Alert[];
  todayTimeline: Array<{
    id: string;
    type: 'nudge' | 'reminder';
...
export const useHomeSummary = () =>
  useQuery({
    ...
    queryFn: async (): Promise<HomeSummaryPayload> => {
      const res = await request<{
      ...
      const p = res.profile ?? {};
      const snapshot = res.deviceStatus?.snapshot ?? {};
      const device = res.deviceStatus?.device ?? {};

      const todayTimeline = (res.timeline ?? []).map((event, idx) => {
        const timestampMs = Number(event.timestampMs ?? 0);
        const scheduledTime = typeof event.scheduledTime === 'string' ? event.scheduledTime : undefined;
        return {
          id: String(event.id ?? `evt-${idx}`),
          type: ((event.type as 'nudge' | 'reminder') ?? 'nudge'),
          title: String(event.title ?? 'Event'),
          subtitle: typeof event.subtitle === 'string' ? event.subtitle : undefined,
          time:
            scheduledTime ??
            (timestampMs > 0
              ? new Date(timestampMs).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
              : '--:--'),
          icon: String(event.icon ?? 'Clock'),
          color: String(event.color ?? 'mint')
        };
      });
      ...
      return {
        ...
        alerts: (res.alerts ?? []).map(toAlert).sort((a, b) => {
          const sevDiff = severityOrder[a.severity] - severityOrder[b.severity];
          if (sevDiff !== 0) return sevDiff;
          return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
        }),
